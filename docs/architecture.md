...
`static/` is generated by the npm build, **committed**, and **compiled into the binary** by
`assets.rs`. Committing it is what lets `cargo run` produce a styled page on a machine with
no Node toolchain; embedding it is what lets the binary be copied anywhere — into a
container, onto a server — and still have one. It holds two bundles: `app.*` for the dashboard,
